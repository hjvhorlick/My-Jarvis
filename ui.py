...
class CustomizeOverlay(QWidget):
    ...
    saved = pyqtSignal(str, str, str, str, str)   # assistant_name, user_name, ui_color, voice, style  @@VOICE_UPGRADE@@
    _OW, _OH = 400, 660
    ...
    def __init__(self, assistant_name="JARVIS", user_name="",
        ...
        # ── Assistant voice — all Gemini voices + speaking style + preview ──
        from memory.config_manager import (AVAILABLE_VOICES, DEFAULT_VOICE, VOICE_INFO,
                                           VOICE_STYLES, get_voice_style)
        _combo_ss = (
            f"QComboBox {{ background: #000d12; color: {C.TEXT}; border: 1px solid {C.BORDER}; "
            f"border-radius: 3px; padding: 3px 6px; font-family: 'Courier New'; font-size: 9pt; }}"
            f"QComboBox:hover {{ border-color: {C.PRI}; }}"
            f"QComboBox QAbstractItemView {{ background: #000d12; color: {C.TEXT}; "
            f"selection-background-color: {C.PRI_GHO}; selection-color: {C.PRI}; }}"
        )
        lay.addSpacing(4)
        lay.addWidget(_lbl("ASSISTANT VOICE  —  timbre", 8, color=C.TEXT_DIM,
                            align=Qt.AlignmentFlag.AlignLeft))
        ...
        for _v in AVAILABLE_VOICES:
            g, desc = VOICE_INFO.get(_v, ("?", ""))
            self._voice_combo.addItem(f"{'♂' if g == 'M' else '♀'}  {_v}  —  {desc}", _v)
        self._voice_combo.setCurrentIndex(max(0, AVAILABLE_VOICES.index(self._sel_voice)))
        self._voice_combo.currentIndexChanged.connect(
            lambda i: self._on_voice_pick(self._voice_combo.itemData(i)))
        lay.addWidget(self._voice_combo)

        lay.addSpacing(4)
        lay.addWidget(_lbl("SPEAKING STYLE  —  accent & attitude", 8, color=C.TEXT_DIM,
                            align=Qt.AlignmentFlag.AlignLeft))
        self._sel_style = get_voice_style()
        self._style_combo = QComboBox()
        self._style_combo.setStyleSheet(_combo_ss)
        self._style_combo.setFixedHeight(30)
        for _st in VOICE_STYLES:
            self._style_combo.addItem(_st, _st)
        _si = list(VOICE_STYLES).index(self._sel_style) if self._sel_style in VOICE_STYLES else 0
        self._style_combo.setCurrentIndex(_si)
        self._style_combo.currentIndexChanged.connect(
            lambda i: setattr(self, "_sel_style", self._style_combo.itemData(i)))
        ...
        self._preview_btn.setFont(QFont("Courier New", 8, QFont.Weight.Bold))
        self._preview_btn.setCursor(Qt.CursorShape.PointingHandCursor)
        self._preview_btn.setStyleSheet(
            f"QPushButton {{ background: {C.PRI_GHO}; color: {C.PRI}; border: 1px solid {C.PRI}; "
        ...
        self._preview_lbl = _lbl("speaks a sample line with the chosen voice + style", 7,
                                 color=C.TEXT_DIM, align=Qt.AlignmentFlag.AlignLeft)
    ...
    def _set_preview_status(self, text: str, enabled: bool = True):
        self._preview_lbl.setText(text)
        self._preview_btn.setEnabled(enabled)

    def _preview_voice(self):
        """Speak one sample line with the selected voice + style via Gemini TTS."""
        voice, style = self._sel_voice, self._sel_style
        self._set_preview_status("generating…", enabled=False)
        ...
        def _work():
            try:
                ...
                from memory.config_manager import VOICE_STYLES, get_gemini_key
                user = (self._user_input.text().strip() or "sir")
                style_txt = VOICE_STYLES.get(style, "")
                line = (f"Hello {user}, what are we working on today? "
                        f"Systems are online and I am at your disposal.")
                prompt = ((style_txt + " ") if style_txt else "") + "Say naturally: " + line
                client = _g.Client(api_key=get_gemini_key())
                resp = None
                    ...
                    try:
                        resp = client.models.generate_content(
                            ...
                            config=_t.GenerateContentConfig(
                                ...
                                speech_config=_t.SpeechConfig(voice_config=_t.VoiceConfig(
                                    prebuilt_voice_config=_t.PrebuiltVoiceConfig(voice_name=voice)))))
                        break
                ...
                QTimer.singleShot(0, lambda: self._set_preview_status(f"playing {voice}…", False))
                _sd.play(audio, samplerate=24000, blocking=True)
                QTimer.singleShot(0, lambda: self._set_preview_status("done — pick another or SAVE"))
            except Exception as e:
                msg = str(e)[:90]
                QTimer.singleShot(0, lambda: self._set_preview_status(f"preview failed: {msg}"))

        threading.Thread(target=_work, daemon=True).start()


    def _refresh_voice_btns(self):
        """No-op after the voice upgrade (dropdown replaces pills)."""
        return

    def _refresh_voice_btns_old(self):
...
class MainWindow(QMainWindow):
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("Vincent's build", C.PRI_DIM))
        return w
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = "",
        ...
        if voice:
            ...
            if voice != get_voice():
                ...
                voice_changed = True
        if style:
            from memory.config_manager import get_voice_style, save_voice_style
            if style != get_voice_style():
                save_voice_style(style)
                voice_changed = True   # style lives in the system prompt → reconnect
